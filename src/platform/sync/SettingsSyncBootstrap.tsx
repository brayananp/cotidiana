...
import { runSettingsSync } from "./settings-sync-client";
import { subscribeToSettingsSyncRequests } from "./settings-sync-events-client";
import { createSyncRuntimeId } from "./sync.types";

const PERIODIC_SYNC_MS = 60_000;

export function SettingsSyncBootstrap({ access }: { access: AppAccess }) {
	...
	useEffect(() => {
		...
		const run = async () => {
			if (disposed || !canSynchronize || !browserReportsOnline()) {
				return;
			}

			try {
				await runSettingsSync({
					userId,
					deviceId,
				});
			} catch {
		...
		const unsubscribeRequests = subscribeToSettingsSyncRequests(() => {
			void run();
		});
