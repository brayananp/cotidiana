...
import {
	pullSchedulingChangesFn,
	pushSchedulingOperationsFn,
} from "./scheduling-sync.functions";
import {
...
export type RunSchedulingSyncInput = {
	userId: string;
	deviceId: string;
};

export type RunSchedulingSyncResult = {
	pushed: number;
	pulled: number;
	conflicts: number;
	rejected: number;
	...
	await db.transaction("rw", db.syncOperations, async () => {
		for (const operation of operations) {
			if (
				...
				!isSchedulingEntityType(operation.entityType) ||
...
async function claimPushBatch(
	...
	return db.transaction("rw", db.syncOperations, db.syncMetadata, async () => {
		...
		const conflictedKeys = new Set(
			conflictMetadata
				.filter((metadata) => isSchedulingEntityType(metadata.entityType))
...
function toPushInput(operation: SyncOperationRecord) {
	if (!isSchedulingEntityType(operation.entityType)) {
		throw new Error("INVALID_SCHEDULING_ENTITY");
	}
...
async function applyPushedSnapshot(
	...
	await db.transaction(
		...
		db.syncMetadata,
		async () => {
			if (operation.entityType === "time_block") {
	...
	await db.transaction(
		...
		async () => {
			for (const change of changes) {
				...
				if (change.entityType === "time_block") {
					await db.timeBlocks.put(snapshot as TimeBlockRecord);
				} else {
					await db.calendarEvents.put(snapshot as CalendarEventRecord);
				}
...
function parseSnapshot(
	...
	value: unknown,
): TimeBlockSyncSnapshot | CalendarEventSyncSnapshot {
	if (entityType === "time_block") {
		return timeBlockSyncSnapshotSchema.parse(value);
	}

	if (entityType === "calendar_event") {
		return calendarEventSyncSnapshotSchema.parse(value);
	}
...
function isSchedulingEntityType(
	value: SyncEntityType,
): value is "time_block" | "calendar_event" {
	return value === "time_block" || value === "calendar_event";
}

function getErrorMessage(error: unknown): string {
	return error instanceof Error ? error.message : "SCHEDULING_SYNC_FAILED";
}
