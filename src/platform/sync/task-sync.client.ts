...
import {
  pullTaskChangesFn,
  pushTaskOperationsFn,
} from './task-sync.functions'
  ...
  return withTaskSyncLock(async () => {
    ...
    try {
      ...
      for (
        ...
        const operations = await claimPushBatch(
          input.userId,
          input.deviceId,
        )
      ...
      for (
        ...
        const cursor = await getTaskCursor(
          input.userId,
        )
  ...
  await db.transaction(
    ...
    async () => {
      ...
      for (const operation of operations) {
        ...
        await db.syncOperations.update(
          operation.id,
          {
            status: 'failed',
...
async function ensureRemoteDevice(
  ...
  if (identity) {
    await db.localIdentities.update(
      identity.id,
      {
        remoteRegisteredAt:
...
function isEligibleForRetry(
  ...
  return (
    new Date(operation.nextRetryAt).getTime() <=
    now.getTime()
  )
}

function toPushInput(
  operation: SyncOperationRecord,
) {
  return {
...
async function applyPushResults(
  ...
  const claimedById = new Map(
    claimedOperations.map((operation) => [
      operation.id,
      operation,
    ]),
  )
  ...
  await db.transaction(
    ...
    async () => {
      for (const result of results) {
        ...
        const metadataId = createSyncMetadataId(
          'task',
          claimed.entityId,
        )
...
async function applySuccessfulPush(
  ...
  const db = getLocalDatabase()
  const snapshot = taskSyncSnapshotSchema.parse(
  ...
  const currentTask = await db.tasks.get(
    claimed.entityId,
  )

  if (newerOperations.length === 0) {
    await db.tasks.put(
      snapshotToTaskRecord(snapshot),
    )
  }
...
async function getTaskCursor(
  userId: string,
): Promise<number> {
  const db = getLocalDatabase()
  const cursor = await db.syncCursors.get(
    createSyncCursorId(userId, 'task'),
  )
  ...
  await db.transaction(
    ...
    async () => {
      for (const change of changes) {
        ...
        const metadata =
          await db.syncMetadata.get(metadataId)

        if (
          change.version <=
          (metadata?.remoteVersion ?? 0)
        ) {
          continue
        }
          ...
          for (const operation of localOperations) {
            await db.syncOperations.update(
              operation.id,
              {
                ...
                nextRetryAt: null,
                lastError:
                  'REMOTE_CHANGE_WITH_LOCAL_PENDING',
                updatedAt: now,
              },
            )
          }
