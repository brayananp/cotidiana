...
export function createTaskCommand(writeStore: TaskWriteStore) {
	return async (rawInput: unknown, context: TaskExecutionContext) => {
		...
		const task = createTaskEntity({
			...
			plannedAt: localDateTimeToIso(input.plannedAt),
			dueAt: localDateTimeToIso(input.dueAt),
		});

		await writeStore.commit(task, "create", context.deviceId);
