...
export class DexieTaskWriteStore implements TaskWriteStore {
	async commit(
		task: Task,
		...
		return db.transaction(
			...
			db.syncMetadata,
			async () => {
				const pendingOperations = await db.syncOperations
				...
				if (requestedOperation === "delete" && pendingCreate) {
					...
					await db.syncMetadata.delete(createSyncMetadataId("task", task.id));
				...
				const effectiveOperation =
					pendingCreate && requestedOperation === "update"
						? "create"
						: requestedOperation;

				const metadataId = createSyncMetadataId("task", task.id);
				...
				const operation: SyncOperationRecord = {
					...
					payload:
						effectiveOperation === "delete"
							...
							: task,
					baseVersion: existingMetadata?.remoteVersion ?? null,
					status: "pending",
					attempts: 0,
					nextRetryAt: null,
					lastError: null,
					createdAt: pendingOperations[0]?.createdAt ?? now,
					updatedAt: now,
				};
				...
				await db.tasks.put(taskToRecord(task));
