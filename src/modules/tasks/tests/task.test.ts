...
describe("Task", () => {
	it("normaliza los datos al crear", () => {
		...
		expect(task.title).toBe("Programar módulo Tasks");

		expect(task.description).toBe("Implementar dominio");
	...
	it("marca la fecha de finalización", () => {
		...
		expect(completed.completedAt).toBe("2026-07-27T13:00:00.000Z");
		expect(completed.version).toBe(2);
	});

	it("elimina completedAt al reabrir", () => {
		const completed = changeTaskStatus(createTask(), "done", now);

		const reopened = changeTaskStatus(completed, "todo", now);

		expect(reopened.completedAt).toBeNull();
	});

	it("archiva sin eliminar", () => {
		const archived = setTaskArchived(createTask(), true, now);
	...
	it("aplica eliminación lógica", () => {
		const deleted = deleteTaskEntity(createTask(), now);
